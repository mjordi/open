...
describe('AccessManagement', function () {
  ...
  describe('Authorization Management', function () {
    ...
    it('Should handle authorization listing', async function () {
      ...
      for (let i = 0; i < authCount; i++) {
        authList.push(
          await accessManagement.getAssetAuthorizationAtIndex(testAssetKey, i)
        );
      }
      expect(authList).to.have.members([
        user1.address,
        user2.address,
        user3.address,
      ]);
  ...
  describe('Access Control', function () {
    ...
    it('Should grant access to asset owner', async function () {
      ...
      expect(await accessManagement.getAccess.staticCall(testAssetKey)).to.be
        .true;
    });
    ...
    it('Should grant access to authorized users', async function () {
      ...
      expect(
        await accessManagement.connect(user1).getAccess.staticCall(testAssetKey)
      ).to.be.true;
    });
    ...
    it('Should deny access to unauthorized users', async function () {
      ...
      expect(
        await accessManagement.connect(user2).getAccess.staticCall(testAssetKey)
      ).to.be.false;
    });
    ...
    it('Should handle access for non-existent assets', async function () {
      ...
      expect(await accessManagement.getAccess.staticCall(nonExistentKey)).to.be
        .false;
    });
  });
  ...
  describe('Asset Queries', function () {
    ...
    it('Should return empty authorization for non-existent or unauthorized users', async function () {
      const role1 = await accessManagement.getAssetAuthorization(
        assetKey,
        user2.address
      );
      const role2 = await accessManagement.getAssetAuthorization(
        'non-existent',
        user1.address
      );
  ...
  describe('Complex Scenarios', function () {
    it('Should handle multiple assets with different owners and authorizations', async function () {
      ...
      await accessManagement
        .connect(user1)
        .addAuthorization(asset1Key, user2.address, 'editor');
      ...
      await accessManagement
        .connect(user2)
        .addAuthorization(asset2Key, user1.address, 'viewer');

      // Assertions for asset 1
      expect(
        await accessManagement.connect(user1).getAccess.staticCall(asset1Key)
      ).to.be.true;
      expect(
        await accessManagement.connect(user2).getAccess.staticCall(asset1Key)
      ).to.be.true;
      expect(
        await accessManagement.connect(user3).getAccess.staticCall(asset1Key)
      ).to.be.false;

      // Assertions for asset 2
      expect(
        await accessManagement.connect(user1).getAccess.staticCall(asset2Key)
      ).to.be.true;
      expect(
        await accessManagement.connect(user2).getAccess.staticCall(asset2Key)
      ).to.be.true;
      expect(
        await accessManagement.connect(user3).getAccess.staticCall(asset2Key)
      ).to.be.false;
    });
    ...
    it('Should handle authorization cascading', async function () {
      ...
      await accessManagement
        .connect(user1)
        .addAuthorization(assetKey, user2.address, 'user');

      // user2, being an authorized user, CAN add user3
      await expect(
        accessManagement
          .connect(user2)
          .addAuthorization(assetKey, user3.address, 'viewer')
      ).to.not.be.reverted;

      // user1 removes user2's access
      await accessManagement
        .connect(user1)
        .removeAuthorization(assetKey, user2.address);
      expect(
